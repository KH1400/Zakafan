...
interface PublicFileUploadComponentProps {
  multiple?: boolean;
  accept?: string;
...
export default function PublicFileUploadComponent({
  multiple = true,
  accept,
  ...
  children
}: PublicFileUploadComponentProps) {
  ...
  const handleDrop = useCallback((e: React.DragEvent) => {
    ...
    if (e.dataTransfer.files && e.dataTransfer.files[0]) {
      ...
      fileUploader.handleFileSelect(mockEvent);
    }
  }, [fileUploader]);

  const handleFileChange = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    fileUploader.handleFileSelect(e);
  }, [fileUploader]);
              ...
              >
                ...
                <div className="flex items-center space-x-2 space-x-reverse">
                  ...
                  {(fileMeta.status === 'idle' || fileMeta.status === 'error') && (
                    <button
                      onClick={() => fileUploader.uploadPublicFile(fileMeta)}
