        ...
        <div className="absolute inset-0 start-1/2 translate-x-1/2 md:m-6 w-full p-6 md:p-0 md:max-w-3xl lg:max-w-4xl xl:max-w-5xl rounded-2xl overflow-hidden flex items-center justify-center">
          <div className={`h-full w-full space-y-4 p-6 bg-muted rounded-2xl overflow-hidden overflow-y-auto shadow-lg shadow-black`}>
            <FileUploadComponent
              ...
              >
              {(postCategory?.image)?(<div className="w-full flex flex-wrap justify-center items-center gap-3 border h-16 overflow-y-auto bg-gray-900/50 relative p-1">
                <div className="h-full relative w-20 border">
                  <Image alt={postCategory.image.id.toString()} src={postCategory.image.file_url} fill style={{ objectFit: "cover" }}></Image>
                  <Button onClick={() => setPostCategory(prev => ({...prev, image: null}))} variant='ghost' className='absolute top-2 end-2 w-4 h-4'><X/></Button>
                </div>
              </div>):null}
            </FileUploadComponent>
            <div className="flex justify-start items-center gap-4">
              <p>اولویت:</p>
              <Input onChange={(e) => setPostCategory(prev => ({...prev, order: +e.target.value}))}
              value={postCategory?.order}
              type='number'
              onKeyDown={(e) => {
                const allowedKeys = [
                  "Backspace",
            ...
            <div className="grid md:grid-cols-4 gap-3">
              {languages.map((language, index) => 
                <div key={index} className="border border-gray-500 p-1 rounded-lg space-y-3">
                  ...
                  <MultiLangInput title="عنوان" text={postCategory?.title[language.lang]} onChange={(e) => {setPostCategory({...postCategory, title: {...postCategory?.title, [language.lang]: e}})}} className="border rounded-md border-muted p-1" />
                  <MultiLangInput title="توضیحات" text={postCategory?.description[language.lang]} onChange={(e) => {setPostCategory({...postCategory, description: {...postCategory?.description, [language.lang]: e}})}} className="border rounded-md border-muted p-1" />
                </div>
              )}
            </div>
            <div className="w-full flex justify-end items-center gap-6">
              <Button variant='destructive' size='lg' disabled={loading} onClick={onClose}>بستن</Button>
              <Button variant='default' size='lg' disabled={loading} onClick={() => {onSubmit(postCategory)}}><Loader2 className={`animate-spin repeat-infinite ${!loading && "hidden"}`}/><span>ذخیره</span></Button>
            </div>
          </div>
        </div>
      </div>
