...
import { apiCreatePostCategory, apiDeletePostCategory, apiGetPostCategories, apiUpdatePostCategory } from '../../../lib/api';
...
const CategoryListPage = () => {
  ...
  const handleInsert = async (category: DynoCategory) => {
    ...
    try {
      const response = await apiCreatePostCategory(cateTemp)
      getCategories();
  ...
  const handleDelete = async (category: DynoCategory) => {
    if (confirm('آیا مطمئن هستید که می‌خواهید این دسته را حذف کنید؟')) {
      try {
        const response = await apiDeletePostCategory(category)
        getCategories();
      } catch (error) {
  ...
  const handleUpdate = async (category: DynoCategory) => {
    ...
    try {
      const response = await apiUpdatePostCategory(cateTemp)
      getCategories();
      setEdittingCategory(null)
      setShowNewModal(false)
      toast({
